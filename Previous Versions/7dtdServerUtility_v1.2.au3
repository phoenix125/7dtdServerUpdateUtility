...
Func UpdateCheck()
	...
	Local $aInstalledVersion = GetInstalledVersion($serverdir)
		If $g_bDebug Then
			FileWriteLine($g_c_sLogFile, _NowCalc() & " . . . Installed version: (" & $aInstalledVersion[1] & ").")
			FileWriteLine($g_c_sLogFile, _NowCalc() & " . . . Latest version:    (" & $aLatestVersion[1] & ").")
		Else
	    EndIf
...
;**** Retrieve Data From serverconfig.xml File ****
   ...
   Local $sFileExists = FileExists($sConfigPath)
	If $sFileExists = 0 Then
			MsgBox(0x0, $ConfigFile & " Not Found", "Could not find " & $sConfigPath)
			FileWriteLine($g_c_sLogFile, _NowCalc() & "!!! ERROR !!! Could not find " & $sConfigPath) & ". Program terminated."
			Exit
    Else
    EndIf
...
;	Local $sManifestExists = FileExists($steamcmddir & "\steamapps\appmanifest_294420.acf")
;	If $sManifestExists = 1 Then
;	    FileMove ($steamcmddir & "\steamapps\appmanifest_294420.acf", $serverdir & "\steamapps\appmanifest_294420.acf", 1)
;				FileWriteLine($g_c_sLogFile, _NowCalc() & " Notice: Install manifest found at " & $steamcmddir & "\steamapps\appmanifest_294420.acf & moved to " & $serverdir & "\steamapps\appmanifest_294420.acf")
;	EndIf
Else
