...
Func UpdateIni($sIniFile)
	...
	IniWrite($sIniFile, "Use Enabled Discord/MCRCON/Twitch Bot to Announce Server SCHEDULED Restarts? (yes/no)", "AnnounceScheduledMessage", $sAnnounceScheduledMessage)
	...
	IniWrite($sIniFile, "Use Enabled Discord/MCRCON/Twitch Bot to Announce Server UPDATE Restarts? (yes/no)", "AnnounceUpdateMessage", $sAnnounceUpdateMessage)
...
While True ;**** Loop Until Closed ****
	...
	If Not ProcessExists($g_s7dtdPID) Then
		...
		Else
			...
			SplashTextOn("7dtdServerUtil", "Restarting server to apply config change(s)." & @CRLF & "Server name: " & $aServerNameVer & @CRLF & "Game Name: " & $aGameName & @CRLF & @CRLF & "Please wait one minute.", 450, 125, -1, -1, $DLG_MOVEABLE, "")
			Global $aRebootMe = 1
			CloseServer()
		EndIf
		; **** Append Server Version to Server Name And/Or Change GameName to Server Version ****
	...
	If ($sUseDiscordBot = "yes") Or ($sUseTwitchBot = "yes") Or ($g_sUseMCRCON = "yes") or $sInGameAnnounce = "yes" Then
		If $g_iBeginDelayedShutdown = 1 Then
			...
			Local $sInGameMsg = """WARNING! Server restarting in " & $g_iDelayShutdownTime & " minute(s)...""" & @CRLF
			If $sAnnounceScheduled = "yes" And $aBotMsg = $sAnnounceScheduledMessage Then
				If $sInGameAnnounce = "yes" Then
					SendInGame($sInGameMsg)
					FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] [RCON] In Game Restart Message Sent to Players")
				EndIf
				If $g_sUseMCRCON = "yes" Then
			...
			If $sAnnounceUpdate = "yes" And $aBotMsg = $sAnnounceUpdateMessage Then
				If $sInGameAnnounce = "yes" Then
					SendInGame($sInGameMsg)
					FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] [RCON] In Game Restart Message Sent to Players")
				EndIf
		...
		ElseIf $g_iBeginDelayedShutdown = 2 And ((_DateDiff('n', $g_sTimeCheck0, _NowCalc())) >= ($g_iDelayShutdownTime - 1)) Then
			...
			If $sInGameAnnounce = "yes" Then
				SendInGame($sInGameMsg)
				FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] [RCON] In Game Shutdown Message Sent to Players")
			EndIf
			If $sAnnounceScheduled = "yes" And $aBotMsg = $sAnnounceScheduledMessage Then
				If $sInGameAnnounce = "yes" Then
					SendInGame($sInGameMsg)
