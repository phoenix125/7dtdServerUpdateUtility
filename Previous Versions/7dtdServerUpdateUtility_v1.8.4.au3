...
#AutoIt3Wrapper_Outfile=Builds\7dtdServerUpdateUtility_v1.8.4.exe
#AutoIt3Wrapper_Outfile_x64=Builds\7dtdServerUpdateUtility_x64_v1.8.4.exe
...
$aUtilVersion = "v1.8.4" ; (2019-02-02)
...
If FileExists($g_c_sPIDFile) Then
	Global $aServerPID = FileRead($g_c_sPIDFile)
Else
	Global $aServerPID = "0"
EndIf
...
Func GetLatestVersion($sCmdDir)
	...
	Else
		...
		If $aServerVer = 0 Then
			FileWriteLine($aLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $aServerPID & ")] Update Check via 7DTD Stable Branch. Latest version: " & $hBuildID)
		Else
			FileWriteLine($aLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $aServerPID & ")] Update Check via 7DTD Experimental Branch. Latest version: " & $hBuildID)
		EndIf
	EndIf
	FileClose($hFileOpen)
...
If $UseRemoteRestart = "yes" Then
	...
	If $MainSocket = -1 Then
		MsgBox(0x0, "TCP Error", "Could not bind to [" & $aServerIP & "] Check server IP or disable Remote Restart in INI")
...
While True ;**** Loop Until Closed ****
	...
	If Not ProcessExists($aServerPID) Then
		...
		If ($aRebootMe = "yes") Or ($aServerTelnetReboot = "yes") Then
			...
			FileWriteLine($ConfigFileTempFull, "</ServerSettings>")
			FileWriteLine($aLogFile, _NowCalc() & " Restarting server to apply config change(s).")
		...
		EndIf
		FileWrite($g_c_sPIDFile, $aServerPID)
