...
#AutoIt3Wrapper_Outfile=Builds\7dtdServerUpdateUtility_v2.1.0.exe
#AutoIt3Wrapper_Outfile_x64=Builds\7dtdServerUpdateUtility_x64_v2.1.0.exe
...
$aUtilVersion = "v2.1.0" ; (2019-02-09)
...
Func Gamercide()
	If @exitMethod <> 1 Then
		...
		If $Shutdown = 6 Then
			FileWriteLine($aLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $aServerPID & ")] Server Shutdown - Intiated by User when closing " & $aUtilName & " Script")
...
Func CloseServer($ip, $port, $pass)
	If $aRebootConfigUpdate = "no" Then
		SplashTextOn($aUtilName & ": " & $aServerName, "Shutting down 7 Days to Die server . . .", 350, 50, -1, -1, $DLG_MOVEABLE, "")
	EndIf
	$aRebootConfigUpdate = "no"
	$aAnnounceCount1 = 0
	$aFPCount = 0
...
Func SendDiscordMsg($sHookURLs, $sBotMessage, $sBotName = "", $sBotTTS = False, $sBotAvatar = "")
	...
	For $i = 1 To $aHookURLs[0]
		...
		Local $sResponseText = ""
		If Not $xDebug Then
			FileWriteLine($aLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $aServerPID & ")] [Discord Bot] Message sent: " & $sBotMessage)
		Else
...
Func GetLatestVersion($sCmdDir)
	...
	If $hFileOpen = -1 Then
		$aReturn[0] = False
	Else
	If StringInStr($hFileRead1, "FAILED with result code") <> 0 Then
			FileWriteLine($aLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $aServerPID & ")] [Update Check] SteamCMD replied with FAILED result code. Ignoring this entry.")
			FileWrite(@ScriptDir & "\" & $aUtilName & "_" & $sAppInfoTemp, $hFileRead1) ; TEST!!!
		$aReturn[0] = True
		$aReturn[1] = $hBuildID
		Return $aReturn
	EndIf
...
Func UpdateCheck()
	...
	ElseIf Not $aLatestVersion[0] And Not $aInstalledVersion[0] Then
		...
		SplashTextOn($aUtilName & ": " & $aServerName & $aServerName, "Something went wrong retrieving Latest & Installed Versions." & @CRLF & "- Running update with -validate" & @CRLF & @CRLF & "(Restart will be delayed if 'announce restart' is enabled)", 500, 125, -1, -1, $DLG_MOVEABLE, "")
		$bUpdateRequired = True
	ElseIf Not $aInstalledVersion[0] Then
		...
		SplashTextOn($aUtilName & ": " & $aServerName, "Something went wrong retrieving Installed Version." & @CRLF & "(This is normal for new install)" & @CRLF & "- Running update with -validate" & @CRLF & @CRLF & "(Restart will be delayed if 'announce restart' is enabled)", 450, 175, -1, -1, $DLG_MOVEABLE, "")
		$bUpdateRequired = True
	ElseIf Not $aLatestVersion[0] Then
		...
		SplashTextOn($aUtilName & ": " & $aServerName, "Something went wrong retrieving Latest Version." & @CRLF & "- Running update with -validate" & @CRLF & @CRLF & "(Restart will be delayed if 'announce restart' is enabled)", 450, 125, -1, -1, $DLG_MOVEABLE, "") 5
		$bUpdateRequired = True
	EndIf
...
#Region ;**** Fail-Proof Script ****
Func FPRun()
Local $tConfigPath = $aServerDirLocal & "\" & $aConfigFile
;Local $aFPConfigBackup = $tConfigPath & "_" & StringRegExpReplace(_NowCalc(), "[\\\/\: ]", "_") & ".bak"
...
EndIf
;If FileExists($tConfigPath) Then
;	FileMove($tConfigPath, $aFPConfigBackup, 1)
;EndIf
...
;	FileWriteLine($tConfigPath, "<property name=""ServerName"" value=""" & $aServerName & """/>")
...
;	GetfromServerConfig()
;	FileCopy($tConfigPath, $aConfigFileTempFull, 1)
	$aFPCount = 0
EndFunc


#Region ;**** Retrieve Data From serverconfig.xml File ****
...
EndFunc
#EndRegion ;**** Retrieve Data From serverconfig.xml File ****
...
;
	...
	If $iniCheck = $sAnnounceNotifyUpdate Then
		$sAnnounceNotifyUpdate = "1,2,5,10"
		$iIniFail += 1
	EndIf
...
;	FileWriteLine($sIniFile, "If YES to either two questions below, this utility will make a backup of your existing serverconfig file (as listed in Game Server Configuration section),)")
	...
	FileWriteLine($sIniFile, "  ServerWebsiteURL,, ServerDisabledNetworkProtocols, GameWorld, WorldGenSeed, WorldGenSize, GameName, GameDifficulty, AdminFileName, DropOnDeath")
...
SplashTextOn($aUtilName & ": " & $aServerName, "7dtdServerUpdateUtility started.", 250, 50, -1, -1, $DLG_MOVEABLE, "")
...
;			FileWriteLine($aLogFile, _NowCalc() & " TEST! a: rebootme = " & $aRebootMe & ". Servertelnmetreboot = " & $aServerTelnetReboot & ".  First boot = " & $aFirstBoot)
		If ($aRebootMe = "yes") Or ($aServerTelnetReboot = "yes") Then
			...
			SplashTextOn($aUtilName & ": " & $aServerName, "Restarting server to apply config change(s)." & @CRLF & "Server name: " & $aServerNameVer & @CRLF & "Game Name: " & $aGameName & @CRLF & @CRLF & "Reboot Reason(s):" & @CRLF & $aServerRebootReason & @CRLF & "Please wait one minute.", 600, 350, -1, -1, $DLG_MOVEABLE, "")
		...
		If @error Or Not $aServerPID Then
			...
			Select
				...
				Case $iMsgBoxAnswer = 2 ;Cancel
					FileWriteLine($aLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $aServerPID & ")] Server Failed to Start - " & $aUtilName & " Shutdown - Intiated by User")
					Exit
