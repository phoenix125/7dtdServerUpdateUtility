...
#AutoIt3Wrapper_Outfile=Builds\7dtdServerUtility_v1.8.1.exe
#AutoIt3Wrapper_Outfile_x64=Builds\7dtdServerUtility_x64_v1.8.1.exe
...
$aUtilVersion = "v1.8.1" ; (2019-01-20)
...
Global Const $g_c_sUtilityVer = "7dtdServerUtility" & " " & $aUtilVersion
...
Func Gamercide()
	If @exitMethod <> 1 Then
		...
		If $Shutdown = 6 Then
			...
			CloseServer($g_IP, $aServerTelnetPort, $aServerTelnetPass)
		EndIf
...
Func CloseServer($ip, $port, $pass)
	...
	Sleep(250)
	If $socket = -1 Then
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] Shut down server Telnet connection FAILED. No response from Telnet.")
	EndIf
...
Func SendInGame($ip, $port, $pass, $g_sInGameMessage)
	...
	Sleep(250)
	If $socket = -1 Then
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] Send Meesage In Game FAILED. No response from Telnet.")
	EndIf
	TCPSend($socket, @CRLF & $pass & @CRLF)
	For $i = 1 To 90
		$aRecv = TCPRecv($socket, 1024)
		If StringInStr($aRecv, "end session.") Then
			...
			FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] Send Meesage In Game: " & $g_sInGameMessage)
			ExitLoop
		EndIf
	Next
	TCPSend($socket, "exit" & @CRLF)
...
Func TelnetCheck($ip, $port, $pass)
	...
	TCPSend($socket, @CRLF & $pass & @CRLF)
	For $i = 1 To 90
		;		$aRecv = $aRecv & TCPRecv($socket, 1024)
		$aRecv = TCPRecv($socket, 1024)
		If StringInStr($aRecv, "end session.") Then
...
Func GetLatestVersion($sCmdDir)
	...
	Else
		...
		If $ServerVer = 0 Then
			FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] Update Check via 7DTD Stable Branch. Latest version: " & $hBuildID)
		Else
			FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] Update Check via 7DTD Experimental Branch. Latest version: " & $hBuildID)
		EndIf
	EndIf
	FileClose($hFileOpen)
...
Func UpdateCheck()
	...
	If ($aLatestVersion[0] And $aInstalledVersion[0]) Then
		If StringCompare($aLatestVersion[1], $aInstalledVersion[1]) = 0 Then
			FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] Server is Up to Date. Installed Version: " & $aInstalledVersion[1])
		Else
			...
			If $g_sExternalScriptUpdate = "yes" Then
				FileWriteLine($g_c_sLogFile, _NowCalc() & " Executing Script When Restarting For Server Update: " & $g_sExternalScriptUpdateDir & "\" & $g_sExternalScriptUpdateFileName)
				RunWait($g_sExternalScriptUpdateDir & '\' & $g_sExternalScriptUpdateFileName, $g_sExternalScriptUpdateDir, @SW_HIDE)
				FileWriteLine($g_c_sLogFile, _NowCalc() & " Executing Script When Restarting For Server Update Finished. Continuing Server Start.")
			EndIf
	...
	ElseIf Not $aLatestVersion[0] And Not $aInstalledVersion[0] Then
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] Something went wrong retrieving Latest & Installed Versions. Running update with -validate")
	...
	ElseIf Not $aInstalledVersion[0] Then
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] Something went wrong retrieving Installed Version. Running update with -validate. (This is normal for new install)")
	...
	ElseIf Not $aLatestVersion[0] Then
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] Something went wrong retrieving Latest Version. Running update with -validate")
...
Func PurgeLogFile($aServerDir)
	$aPurgeLogFileName = @ScriptDir & "\7dtdServerUtility_PurgeLogFile.bat"
	...
	If $g_bDebug Then
		FileWriteLine($g_c_sLogFile, _NowCalc() & " Deleting server log files >20: " & $aServerDir & "\7DaysToDieServer_Data\output_log_dedi*.txt")
	EndIf
...
Func ReadUini($sIniFile, $sLogFile)
	...
	Global $aTelnetCheck = IniRead($sIniFile, "Use Telnet to Check if Server is Alive?", "TelnetCheck", $iniCheck)
	Global $aTelnetCheckSec = IniRead($sIniFile, "Use Telnet to Check if Server is Alive?", "TelnetCheckInterval(30-900_seconds)", $iniCheck)
	...
	EndIf
	If $AppendVerBegin = "yes" Or $AppendVerEnd = "yes" Or $WipeServer = "yes" Then
		$aRebootMe = "yes"
	EndIf
...
Func iniFileCheck($sIniFile, $iIniFail)
	If FileExists($sIniFile) Then
		...
		UpdateIni($sIniFile)
		FileWriteLine($g_c_sLogFile, _NowCalc() & " INI MISMATCH: Found " & $iIniFail & " missing variable(s) in 7dtdServerUtility.ini. Backup created and all existing settings transfered to new INI. Please modify INI and restart.")
		MsgBox(4096, "INI MISMATCH", "Found " & $iIniFail & " missing variable(s) in 7dtdServerUtility.ini." & @CRLF & @CRLF & "Backup created and all existing settings transfered to new INI." & @CRLF & @CRLF & "Please modify INI and restart.")
		Exit
	Else
		UpdateIni($sIniFile)
		SplashOff()
...
Func UpdateIni($sIniFile)
	...
	IniWrite($sIniFile, "Use Telnet to Check if Server is Alive?", "TelnetCheck", $aTelnetCheck)
	IniWrite($sIniFile, "Use Telnet to Check if Server is Alive?", "TelnetCheckInterval(30-900_seconds)", $aTelnetCheckSec)
...
SplashTextOn("7dtdServerUtil", "7dtdServerUtility started.", 200, 50, -1, -1, $DLG_MOVEABLE, "")
...
If $aServerTelnetEnable = "no" Or $aServerTelnetEnable = "false" Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " . . . Server telnet was disabled. Telnet required for this utility. TelnetEnabled set to: yes")
	$aServerTelnetEnable = "true"
	$aServerTelnetReboot = "yes"
EndIf
If $aServerTelnetPort = "" Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " . . . Server Telnet Port was blank. Port CHANGED to default value: 8081")
	$aServerTelnetPort = "8081"
	$aServerTelnetReboot = "yes"
EndIf
If $aServerTelnetPass = "CHANGEME" Or $aServerTelnetPass = "" Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " . . . Server Telnet Password was " & $aServerTelnetPass & ". Password CHANGED to: 7dtdServerUtility")
	$aServerTelnetPass = "7dtdServerUtility"
	$aServerTelnetReboot = "yes"
EndIf
...
While True ;**** Loop Until Closed ****
	...
	If Not ProcessExists($g_s7dtdPID) Then
		...
		If $g_sExecuteExternalScriptValidate = "yes" Then
			FileWriteLine($g_c_sLogFile, _NowCalc() & " Executing AFTER Update Check External Script " & $g_sExternalScriptDirValidate & "\" & $g_sExternalScriptNameValidate)
			RunWait($g_sExternalScriptDirValidate & '\' & $g_sExternalScriptNameValidate, $g_sExternalScriptDirValidate, @SW_HIDE)
			FileWriteLine($g_c_sLogFile, _NowCalc() & " External AFTER Update Check Script Finished. Continuing Server Start.")
		EndIf

		$LogTimeStamp = "7DaysToDieServer_Data\output_log_dedi" & StringRegExpReplace(_NowCalc(), "[\\\/\: ]", "_") & ".txt"
		If $g_bDebug Then
			FileWriteLine($g_c_sLogFile, _NowCalc() & " 7DTD Dedicated Server Started [" & $serverdir & "\" & $g_c_sServerEXE & " -logfile " & $LogTimeStamp & " -quit -batchmode -nographics -configfile=" & $ConfigFileTemp & " -dedicated ]")
		Else
			FileWriteLine($g_c_sLogFile, _NowCalc() & " 7DTD Dedicated Server Started.")
		EndIf
		SplashOff()
		PurgeLogFile($serverdir)
		$g_s7dtdPID = Run("" & $serverdir & "\" & $g_c_sServerEXE & " -logfile " & $LogTimeStamp & " -quit -batchmode -nographics -configfile=" & $ConfigFileTemp & " -dedicated", $serverdir)
		$gTelnetTimeCheck0 = _NowCalc()
		...
		EndIf
		If $g_bDebug Then
			FileWriteLine($g_c_sLogFile, _NowCalc() & " Server version: " & $aGameVer & ".  Version derived from """ & $serverdir & "\" & $LogTimeStamp & """.")
		Else
			FileWriteLine($g_c_sLogFile, _NowCalc() & " Server version: " & $aGameVer & ".")
		EndIf
		...
		If $aRebootMe = "yes" Or $aServerTelnetReboot = "yes" Then
			...
			Else
				...
				If $g_bDebug Then
					FileWriteLine($g_c_sLogFile, _NowCalc() & " Changing ServerName to """ & $aServerNameVer & """ in " & $ConfigFileTempFull & ".")
				EndIf
			EndIf
			...
			Else
				...
				If $g_bDebug Then
					FileWriteLine($g_c_sLogFile, _NowCalc() & " Changing GameName to """ & $aGameName & """ in " & $ConfigFileTempFull & ".")
				EndIf
			EndIf
			...
			FileWriteLine($ConfigFileTempFull, "</ServerSettings>")
			FileWriteLine($g_c_sLogFile, _NowCalc() & " Restarting server to apply config change(s).")
			FileClose($ConfigFileTempFull)
			SplashTextOn("7dtdServerUtil", "Restarting server to apply config change(s)." & @CRLF & "Server name: " & $aServerNameVer & @CRLF & "Game Name: " & $aGameName & @CRLF & @CRLF & "Please wait one minute.", 600, 100, -1, -1, $DLG_MOVEABLE, "")
			Global $aRebootMe = "no"
	...
	If ($aTelnetCheck = "yes") And (_DateDiff('s', $gTelnetTimeCheck0, _NowCalc()) >= $aTelnetCheckSec) Then
		$TelnetCheckResult = TelnetCheck($g_IP, $aServerTelnetPort, $aServerTelnetPass)
		$gTelnetTimeCheck0 = _NowCalc()
		If $TelnetCheckResult = 0 Then
			If $g_bDebug Then
				FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] Server not responding to telnet. Restarting server.")
			EndIf
			CloseServer($g_IP, $aServerTelnetPort, $aServerTelnetPass)
		Else
			If $g_bDebug Then
				FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] Server responded to telnet Commands.")
			EndIf
		EndIf
	EndIf
	#EndRegion ;**** KeepServerAlive Telnet Check ****
	...
	If ($sUseDiscordBotScheduled = "yes") Or ($sUseDiscordBotUpdate = "yes") Or ($sUseTwitchBotScheduled = "yes") Or ($sUseTwitchBotUpdate = "yes") Or ($sInGameAnnounce = "yes") Then
		If $g_iBeginDelayedShutdown = 1 Then
			...
			Local $sInGameMsgUpdate = """WARNING! New Update. Server restarting in " & $g_iDelayShutdownTime & " minutes...""" & @CRLF
			If $aBotMsg = $sAnnounceScheduledMessage Then
				If $sInGameAnnounce = "yes" Then
					SendInGame($g_IP, $aServerTelnetPort, $aServerTelnetPass, $sInGameMsgDaily)
					;					FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] [RCON] In Game Daily Server Restart Message Sent to Players")
				EndIf
			...
			If $aBotMsg = $sAnnounceUpdateMessage Then
				If $sInGameAnnounce = "yes" Then
					SendInGame($g_IP, $aServerTelnetPort, $aServerTelnetPass, $sInGameMsgUpdate)
					;					FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] [RCON] In Game New Update Restart Message Sent to Players")
				EndIf
		...
		ElseIf $g_iBeginDelayedShutdown = 2 And ((_DateDiff('n', $g_sTimeCheck0, _NowCalc())) >= ($g_iDelayShutdownTime - 1)) Then
			...
			If $aBotMsg = $sAnnounceScheduledMessage Then
				If $sInGameAnnounce = "yes" Then
					SendInGame($g_IP, $aServerTelnetPort, $aServerTelnetPass, $sInGameMsg)
					;					FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] [RCON] In Game 1 Minute Shutdown Message Sent to Players")
				EndIf
				If $sUseDiscordBotScheduled = "yes" Then
					SendDiscordMsg($sDiscordWebHookURLs, $sShutdownMessage, $sDiscordBotName, $bDiscordBotUseTTS, $sDiscordBotAvatar)
				EndIf
