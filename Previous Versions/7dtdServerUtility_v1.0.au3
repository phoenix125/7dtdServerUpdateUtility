...
#AutoIt3Wrapper_Res_Comment=By Phoenix125 based on Dateranoth's 7dtdServerUtility v3.3.0-Beta.3 - June 18, 2018
...
Func Gamercide()
	If @exitMethod <> 1 Then
		$Shutdown = MsgBox(4100, "Shut Down?", "Do you wish to shutdown Server " & $ServerName & "? (PID: " & $g_s7dtdPID & ")", 60)
		If $Shutdown = 6 Then
			FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Server Shutdown - Intiated by User when closing 7dtdServerUtility Script")
			CloseServer()
		EndIf
		MsgBox(4096, "Thanks for using our Application", "Please visit http://www.Phoenix125.com and https://gamercide.com", 5)
		FileWriteLine($g_c_sLogFile, _NowCalc() & " 7dtdServerUtility Stopped by User")
	Else
		FileWriteLine($g_c_sLogFile, _NowCalc() & " 7dtdServerUtility Stopped")
	EndIf
...
Func CloseServer()
   ...
   If FileExists(@ScriptDir & "\puttytel.exe") Then
	  Run(@ScriptDir & "\puttytel.exe -P "& $PuttyPort & " "& $g_IP)
	  ...
	  Else
		 Local $PID = ProcessExists("puttytel.exe")
		 Local $PID2 = ProcessExists("7DaysToDie.exe")
			If $PID Then
			   ProcessClose($PID)
			EndIf
			If $PID2 Then
	  ...
	  EndIf
   Else
      Local $PID = ProcessExists("7DaysToDie.exe")
	  If $PID Then
		 ProcessClose($PID)
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Server Did not Shut Down Properly. Killing Process")
	  EndIf
   EndIf
EndFunc
...
Func _Discord_ErrFunc($oError)
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Error: 0x" & Hex($oError.number) & " While Sending Discord Bot Message.")
...
Func SendDiscordMsg($sHookURLs, $sBotMessage, $sBotName = "", $sBotTTS = False, $sBotAvatar = "")
	...
	For $i = 1 To $aHookURLs[0]
		...
		EndIf
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] [Discord Bot] Message Status Code {" & $oStatusCode & "} " & $sResponseText)
	Next
...
Func TwitchMsgLog($sT_Msg)
	...
	If $aTwitchIRC[0] Then
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] [Twitch Bot] Successfully Connected to Twitch IRC")
		If $aTwitchIRC[1] Then
			FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] [Twitch Bot] Username and OAuth Accepted. [" & $aTwitchIRC[2] & "]")
			If $aTwitchIRC[3] Then
				FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] [Twitch Bot] Successfully sent ( " & $sT_Msg & " ) to all Channels")
			Else
				FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] [Twitch Bot] ERROR | Failed sending message ( " & $sT_Msg & " ) to one or more channels")
			EndIf
		Else
			FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] [Twitch Bot] ERROR | Username and OAuth Denied [" & $aTwitchIRC[2] & "]")
		EndIf
	Else
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] [Twitch Bot] ERROR | Could not connect to Twitch IRC. Is this URL or port blocked? [irc.chat.twitch.tv:6667]")
	EndIf
...
Local $aReturn[2] = [False, ""]
	...
	Else
	  If $ServerVer = 1 Then
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Update Check via 7DTD Experimental Branch")
		...
		EndIf
	  Else
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Update Check via 7DTD Stable Branch")
		Local $sFileRead = FileRead($hFileOpen)
...
Func UpdateCheck()
	If FileExists($steamcmddir & "\app_info.tmp") Then
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Delaying Update Check for 1 minute. | Found Existing " & $steamcmddir & "\app_info.tmp")
		Sleep(60000)
		If FileExists($steamcmddir & "\app_info.tmp") Then
			FileDelete($steamcmddir & "\app_info.tmp")
			FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Deleted " & $steamcmddir & "\app_info.tmp")
		EndIf
	EndIf

	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Update Check Starting.")
	...
	If ($aLatestVersion[0] And $aInstalledVersion[0]) Then
		If StringCompare($aLatestVersion[1], $aInstalledVersion[1]) = 0 Then
			FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Server is Up to Date. Version: " & $aInstalledVersion[1])
		Else
			FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Server is Out of Date! Installed Version: " & $aInstalledVersion[1] & " Latest Version: " & $aLatestVersion[1])
			$bUpdateRequired = True
		EndIf
	ElseIf Not $aLatestVersion[0] And Not $aInstalledVersion[0] Then
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Something went wrong retrieving Latest Version")
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Something went wrong retrieving Installed Version")
...
Func ReadUini($sIniFile, $sLogFile)
	...
	Global $ServerName = IniRead($sIniFile, "Server Name (For Logs & Notifications)", "ServerName", $iniCheck)
	Global $ConfigFile = IniRead($sIniFile, "Server Config File", "ConfigFile", $iniCheck)
	...
	Global $PuttyPort = IniRead($sIniFile, "Telnet Server Port", "PuttyPort", $iniCheck)
	...
	If $iniCheck = $GamePort Then
		...
		$iIniFail += 1
	EndIf
	If $iniCheck = $PuttyPort Then
		$PuttyPort = "8081"
		$iIniFail += 1
	EndIf
	If $iniCheck = $serverdir Then
		$serverdir = "D:\Game_Servers\SteamCMD\steamapps\common\7 Days to Die Dedicated Server"
		$iIniFail += 1
	EndIf
	...
	If $iniCheck = $steamcmddir Then
		$steamcmddir = "D:\Game_Servers\SteamCMD"
		$iIniFail += 1
	EndIf
	...
	If $iniCheck = $g_sMCrconPath Then
		$g_sMCrconPath = "D:\Game_Servers\mcrcon"
		$iIniFail += 1
	EndIf
	...
	If $iniCheck = $sDiscordBotName Then
		$sDiscordBotName = "7 Days To Die Discord Bot"
		$iIniFail += 1
	EndIf
	...
	If $iniCheck = $g_sExternalScriptDir Then
		$g_sExternalScriptDir = "D:\Game_Servers\SQL_Scripts"
		$iIniFail += 1
	EndIf
...
Func UpdateIni($sIniFile)
	IniWrite($sIniFile, "Server Name (For Logs & Notifications)", "ServerName", $ServerName)
	IniWrite($sIniFile, "Server Config File", "ConfigFile", $ConfigFile)
	...
	IniWrite($sIniFile, "Telnet Server Port", "PuttyPort", $PuttyPort)
...
If $UseSteamCMD = "yes" Then
	...
	If $sManifestExists = 1 Then
		Local $manifestFound = MsgBox(4100, "Warning", "Install manifest found at " & $steamcmddir & "\steamapps\appmanifest_294420.acf" & @CRLF & @CRLF & "Suggest moving file to " & _
				$serverdir & "\steamapps\appmanifest_294420.acf before running SteamCMD" & @CRLF & @CRLF & "Would you like to Exit Now?", 20)
...
While True ;**** Loop Until Closed ****
	...
	If $UseRemoteRestart = "yes" Then
		Local $sRestart = _RemoteRestart($MainSocket, $RestartCode, $g_sRKey, $g_IP, $ServerName, $g_bDebug)
		Switch @error
			Case 0

				If ProcessExists($g_s7dtdPID) And ($g_iBeginDelayedShutdown = 0) Then
					Local $MEM = ProcessGetStats($g_s7dtdPID, 0)
					FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] [Work Memory:" & $MEM[0] & " | Peak Memory:" & $MEM[1] & "] " & $sRestart)
					If ($sUseDiscordBot = "yes") Or ($sUseTwitchBot = "yes") Or ($g_sUseMCRCON = "yes") Then
	...
	If Not ProcessExists($g_s7dtdPID) Then
	    ...
	    EndIf
			If $validategame = "yes" Then
				FileWriteLine($g_c_sLogFile, _NowCalc() & " Running SteamCMD with validate. [steamcmd.exe +@ShutdownOnFailedCommand 1 +@NoPromptForPassword 1 +login anonymous +force_install_dir """ & $serverdir & """ validate" & $ServExp & " +quit]")
				RunWait("" & $steamcmddir & "\steamcmd.exe +@ShutdownOnFailedCommand 1 +@NoPromptForPassword 1 +login anonymous +force_install_dir """ & $serverdir & """ validate" & $ServExp & " +quit")
			Else
				FileWriteLine($g_c_sLogFile, _NowCalc() & " Running SteamCMD without validate. [steamcmd.exe +@ShutdownOnFailedCommand 1 +@NoPromptForPassword 1 login anonymous +force_install_dir """ & $serverdir & "" & $ServExp & " +quit")
		...
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Started [" & $g_c_sServerEXE & $LogTimeStamp & " -quit -batchmode -nographics -configfile=" & $ConfigFile & " -dedicated ]")
		...
		If @error Or Not $g_s7dtdPID Then
			...
			Select
				Case $iMsgBoxAnswer = 4 ;Retry
					FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Server Failed to Start. User Initiated a Restart Attempt.")
				Case $iMsgBoxAnswer = 2 ;Cancel
					FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Server Failed to Start - 7dtdServerUtility Shutdown - Intiated by User")
					Exit
				Case $iMsgBoxAnswer = -1 ;Timeout
					FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Server Failed to Start. Script Initiated Restart Attempt after 60 seconds of no User Input.")
			EndSelect
		EndIf
		...
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Window Handle Found: " & $g_h7dtdhWnd)
	ElseIf ((_DateDiff('n', $g_sTimeCheck1, _NowCalc())) >= 5) Then
		Local $MEM = ProcessGetStats($g_s7dtdPID, 0)
		If $MEM[0] > $ExMem And $ExMemRestart = "no" And $g_bDebug Then
			FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Work Memory:" & $MEM[0] & " Peak Memory:" & $MEM[1])
		ElseIf $MEM[0] > $ExMem And $ExMemRestart = "yes" Then
			FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Work Memory:" & $MEM[0] & " Peak Memory:" & $MEM[1] & " Excessive Memory Use - Restart Requested by 7dtdServerUtility Script")
			CloseServer()
		EndIf
	...
	If (($g_sRestartDaily = "yes") And ((_DateDiff('n', $g_sTimeCheck2, _NowCalc())) >= 1) And (DailyRestartCheck($g_sRestartDays, $g_sRestartHours, $g_sRestartMin))  And ($g_iBeginDelayedShutdown = 0) ) Then
		If ProcessExists($g_s7dtdPID) Then
			Local $MEM = ProcessGetStats($g_s7dtdPID, 0)
			FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Work Memory:" & $MEM[0] & " Peak Memory:" & $MEM[1] & " - Daily Restart Requested by 7dtdServerUtility Script")
			If ($sUseDiscordBot = "yes") Or ($sUseTwitchBot = "yes") Or ($g_sUseMCRCON = "yes") Then
				$g_iBeginDelayedShutdown = 1
				$g_sTimeCheck0 = _NowCalc
			Else
	...
	If ($sUseDiscordBot = "yes") Or ($sUseTwitchBot = "yes") Or ($g_sUseMCRCON = "yes") Then
		If $g_iBeginDelayedShutdown = 1 Then
			FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] Bot in Use. Delaying Shutdown for " & $g_iDelayShutdownTime & " minutes. Notifying Channel")
			...
			If $g_sUseMCRCON = "yes" Then
				...
				FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_s7dtdPID & ")] [RCON] Sent ( " & $sShutdownMessage & " ) to Players")
			EndIf
