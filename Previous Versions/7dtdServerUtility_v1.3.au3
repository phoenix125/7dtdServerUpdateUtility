...
If $AppendVerBegin = "no" And $AppendVerEnd = "no" Or $aRebootMe = 1 Then
Else
   $ConfigFileTemp = "ServerConfig7dtdServerUtilTemp.xml"
   $ConfigFileTempFull = $serverdir & "\" & $ConfigFileTemp
   ...
   Local $tConfigRead1 = StringRegExpReplace($tConfigRead2, $aServerName, $aServerNameVer)
   FileWrite($ConfigFileTempFull, $tConfigRead1)
   FileClose($tConfigPath)
   If $g_bDebug Then
	 FileWriteLine($g_c_sLogFile, _NowCalc() & " Appending server version to server name, """ & $aServerNameVer & """ in " & $ConfigFileTempFull & ".")
   EndIf
   FileWriteLine($g_c_sLogFile, _NowCalc() & " Restarting server to apply server name change. New server name: " & $aServerNameVer)
   SplashTextOn("Restarting Server", "Restarting server to apply server name change." & @CRLF & ". . . New server name: " & $aServerNameVer & @CRLF & @CRLF & "Please wait 1-2 minutes.")
   Global $aRebootMe = 1
   CloseServer()
EndIf
