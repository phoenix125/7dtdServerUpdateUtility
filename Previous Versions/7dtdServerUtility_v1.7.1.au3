...
#AutoIt3Wrapper_Outfile=Builds\7dtdServerUtility_v1.7.1.exe
#AutoIt3Wrapper_Outfile_x64=Builds\7dtdServerUtility_x64_v1.7.1.exe
...
Func UpdateCheck()
	...
	ElseIf Not $aInstalledVersion[0] Then
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] Something went wrong retrieving Installed Version. Running update with -validate")
		SplashTextOn("7dtdServerUtil", "Something went wrong retrieving Installed Version." & @CRLF & "- Running update with -validate" & @CRLF & @CRLF & "(Restart will be delayed if 'announce restart' is enabled)", 450, 125, -1, -1, $DLG_MOVEABLE, "")
		Global $aBotMsg = $sAnnounceUpdateMessage
		$bUpdateRequired = True
...
Func ReadUini($sIniFile, $sLogFile)
	...
	Global $g_sUseMCRCONScheduled = IniRead($sIniFile, "Use MCRCON to Announce Scheduled/Update Restarts? (yes/no)", "use_mcrconSCHEDULED", $iniCheck)
	Global $g_sUseMCRCONUpdate = IniRead($sIniFile, "Use MCRCON to Announce Scheduled/Update Restarts? (yes/no)", "use_mcrconUPDATE", $iniCheck)
	Global $g_sMCrconPath = IniRead($sIniFile, "Use MCRCON to Announce Scheduled/Update Restarts? (yes/no)", "mcrconPath", $iniCheck)
	...
	EndIf
	If $iniCheck = $g_sUseMCRCONScheduled Then
		$g_sUseMCRCONScheduled = "no"
		$iIniFail += 1
	ElseIf $g_sUseMCRCONScheduled = "yes" And $g_sRconEnabled <> "yes" Then
		$g_sUseMCRCONScheduled = "no"
	...
	EndIf
	If $iniCheck = $g_sUseMCRCONUpdate Then
		$g_sUseMCRCONUpdate = "no"
		$iIniFail += 1
	ElseIf $g_sUseMCRCONUpdate = "yes" And $g_sRconEnabled <> "yes" Then
		$g_sUseMCRCONUpdate = "no"
		FileWriteLine($sLogFile, _NowCalc() & " Server RCON is Disabled. Disabling MCRCON Notifications. Cannot send RCON message without RCON enabled!")
	EndIf
	...
	If $iniCheck = $g_sExternalScriptDir Then
		$g_sExternalScriptDir = "D:\Game Servers\SteamCMD\steamapps\common\7 Days to Die Dedicated Server\Scripts"
		$iIniFail += 1
	EndIf
	...
	If $iniCheck = $g_sExternalScriptDirValidate Then
		$g_sExternalScriptDirValidate = "D:\Game Servers\SteamCMD\steamapps\common\7 Days to Die Dedicated Server\Scripts"
		$iIniFail += 1
	EndIf
	...
	If ($sUseDiscordBotScheduled = "yes") Or ($sUseDiscordBotUpdate = "yes") Or ($sUseTwitchBotScheduled = "yes") Or ($sUseTwitchBotUpdate = "yes") Or ($g_sUseMCRCONScheduled = "yes") Or ($g_sUseMCRCONUpdate = "yes") or ($sInGameAnnounce = "yes") Then
...
Func UpdateIni($sIniFile)
	...
	IniWrite($sIniFile, "Use MCRCON to Announce Scheduled/Update Restarts? (yes/no)", "use_mcrconSCHEDULED", $g_sUseMCRCONScheduled)
	IniWrite($sIniFile, "Use MCRCON to Announce Scheduled/Update Restarts? (yes/no)", "use_mcrconUPDATE", $g_sUseMCRCONUpdate)
	IniWrite($sIniFile, "Use MCRCON to Announce Scheduled/Update Restarts? (yes/no)", "mcrconPath", $g_sMCrconPath)
...
If $g_sUseMCRCONScheduled = "yes" Then
	...
	EndIf
EndIf
If $g_sUseMCRCONUpdate = "yes" Then
	Local $sFileExists = FileExists($g_sMCrconPath & "\mcrcon.exe")
	If $sFileExists = 0 Then
		InetGet("https://github.com/Tiiffi/mcrcon/releases/download/v0.0.5/mcrcon-0.0.5-windows.zip", @ScriptDir & "\mcrcon.zip", 0)
		DirCreate($g_sMCrconPath) ; to extract to
...
While True ;**** Loop Until Closed ****
	...
	If Not ProcessExists($g_s7dtdPID) Then
		...
		If $g_sExecuteExternalScriptValidate = "yes" Then
			...
			RunWait($g_sExternalScriptDirValidate & '\' & $g_sExternalScriptNameValidate, $g_sExternalScriptDirValidate)
		...
		If $AppendVerBegin = "no" And $AppendVerEnd = "no" And $WipeServer = "no" Or $aRebootMe = 1 Then
		Else
		...
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")]")
	...
	If ($CheckForUpdate = "yes") And ((_DateDiff('n', $g_sTimeCheck0, _NowCalc())) >= $UpdateInterval) And ($g_iBeginDelayedShutdown = 0) Then
		Local $bRestart = UpdateCheck()
		If $bRestart And (($sUseDiscordBotScheduled = "yes") Or ($sUseDiscordBotUpdate = "yes") Or ($sUseTwitchBotScheduled = "yes") Or ($sUseTwitchBotUpdate = "yes") Or ($g_sUseMCRCONScheduled = "yes") Or ($g_sUseMCRCONUpdate = "yes") or ($sInGameAnnounce = "yes")) Then
			$g_iBeginDelayedShutdown = 1
	...
	#Region ;**** Announce to Twitch, MCRCON, In Game, Discord ****
	If ($sUseDiscordBotScheduled = "yes") Or ($sUseDiscordBotUpdate = "yes") Or ($sUseTwitchBotScheduled = "yes") Or ($sUseTwitchBotUpdate = "yes") Or ($g_sUseMCRCONScheduled = "yes") Or ($g_sUseMCRCONUpdate = "yes") or ($sInGameAnnounce = "yes") Then
	...
	Else
		$g_iBeginDelayedShutdown = 0
	EndIf
	#EndRegion ;**** Announce to Twitch, MCRCON, In Game, Discord ****
