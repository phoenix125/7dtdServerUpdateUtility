...
#AutoIt3Wrapper_Outfile=Builds\7dtdServerUpdateUtility_v2.3.2.exe
...
Else
$gWatchdogServerStartTimeCheck = _DateAdd('h', -1, _NowCalc())
EndIf
...
Opt("TrayMenuMode", 3)
Global $iTrayQueryServerName = TrayCreateItem($aServerQueryName)
Global $iTrayQueryPlayers = TrayCreateItem("Players Online: " & $aPlayersCount & " / " & $aPlayersMax)
TrayCreateItem("")
...
Global $gServerUpdatedTimeCheck0 = IniRead($aIniFile, " --------------- SYSTEM USE ONLY --------------- ", "Last Server Update ###", "no")
If $gServerUpdatedTimeCheck0 = "no" Then
$gServerUpdatedTimeCheck0 = _NowCalc()
IniWrite($aIniFile, " --------------- SYSTEM USE ONLY --------------- ", "Last Server Update ###", $gServerUpdatedTimeCheck0)
EndIf
#Region
Local $ServExp = ""
...
Local $tCmd = 'SET steampath=' & $aSteamCMDDir & @CRLF & 'SET gamepath=' & $aServerDirLocal & @CRLF & '%steampath%\steamcmd.exe +@ShutdownOnFailedCommand 1 +@NoPromptForPassword 1 +login anonymous +force_install_dir "%gamepath%" +app_update ' & $aSteamAppID & ' ' & $ServExp & ' ' & $aSteamExtraCMD
...
FileDelete($aBatchDIR & "\Update_7DTD_Beta_Validate_YES.bat")
FileDelete($aBatchDIR & "\Update_7DTD_Beta_Validate_NO.bat")
Local $xArray[85]
$xArray[0] = '@echo off'
...
If _DateDiff('n', $gServerUpdatedTimeCheck0, _NowCalc()) >= $aTelnetDelayAfterUpdate Then
...
$aQueryPort = $aServerPort
For $i = 1 To 6
Local $tQueryCheckResult = _GetQuery($aQueryIP, $aQueryPort)
If UBound($tQueryCheckResult) < 14 Then
If $i = 6 Then
If _DateDiff('n', $gServerUpdatedTimeCheck0, _NowCalc()) >= $aTelnetDelayAfterUpdate Then
If _DateDiff('n', $gWatchdogServerStartTimeCheck, _NowCalc()) >= $aWatchdogWaitServerStart Then
FileWriteLine($aLogFile, _NowCalc() & " [Query] KeepAlive check FAILED ALL 5 COUNTS. Restarting server.")
CloseServer($aTelnetIP, $aTelnetPort, $aTelnetPass)
ExitLoop
EndIf
EndIf
EndIf
Else
$aServerQueryName = $tQueryCheckResult[1]
If $tQueryCheckResult[6] <> $aPlayersCount Then
...
EndFunc
#EndRegion
Func _UpdateQueryInfo()
TrayItemSetText($iTrayQueryServerName, $aServerQueryName)
...
FileWriteLine($aLogFile, _NowCalc() & " [Update] Using SteamDB " & $bServerVer & " branch. Latest version: " & $hBuildID)
EndIf
FileClose($hFileOpen)
...
If $xDebug Then
FileWriteLine($aLogFile, _NowCalc() & " [Update] Update Check via " & $aServerVer & " Branch. Latest version: " & $hBuildID)
EndIf
If FileExists($sFilePath) Then
FileDelete($sFilePath)
EndIf
...
If $xDebug Then
FileWriteLine($aLogFile, _NowCalc() & " Notice: Install manifest found at " & $aSteamCMDDir & "\steamapps\appmanifest_" & $aSteamAppID & ".acf & renamed to appmanifest_" & $aSteamAppID & "_" & $TimeStamp & ".acf")
EndIf
Else
$aFirstBoot = 0
EndIf
...
If $xDebug Then
FileWriteLine($aLogFile, _NowCalc() & " Notice: Install manifest found at " & $aServerDirLocal & "\steamapps\appmanifest_" & $aSteamAppID & ".acf & renamed to appmanifest_" & $aSteamAppID & "_" & $TimeStamp & ".acf")
EndIf
Else
$aFirstBoot = 0
EndIf
...
If $xDebug Then
FileWriteLine($aLogFile, _NowCalc() & " [Steam Update] Running SteamCMD " & $aServerVer & " branch with validate: [" & $aSteamUpdateCMDValY & "]")
Else
FileWriteLine($aLogFile, _NowCalc() & " [Steam Update] Running SteamCMD " & $aServerVer & " branch with validate.")
EndIf
RunWait($aSteamUpdateCMDValY)
Else
If $xDebug Then
FileWriteLine($aLogFile, _NowCalc() & " [Steam Update] Running SteamCMD " & $aServerVer & " branch without validate. [" & $aSteamUpdateCMDValN & "]")
Else
FileWriteLine($aLogFile, _NowCalc() & " [Steam Update] Running SteamCMD " & $aServerVer & " branch without validate.")
EndIf
...
Global $aTelnetDelayAfterUpdate = IniRead($sIniFile, " --------------- KEEP ALIVE WATCHDOG ---------------", "Pause watchdog for _ minutes after server updated to allow map generation (1-360) ###", $iniCheck)
...
Global $sDiscordPlayersMsg = IniRead($sIniFile, " --------------- DISCORD INTEGRATION --------------- ", "Online Player Message (\o - Online Player Count, \m - Max Players) ###", $iniCheck)
...
$aQueryCheckSec = 30
FileWriteLine($aLogFile, _NowCalc() & " [Notice] Query Port server-is-alive check interval was out of range. Interval set to: " & $aQueryCheckSec & " seconds.")
...
FileWriteLine($aLogFile, _NowCalc() & " [Notice] Telnet server-is-alive check interval was out of range. Interval set to: " & $aTelnetCheckSec & " seconds.")
EndIf
If $iniCheck = $aTelnetDelayAfterUpdate Then
$aTelnetDelayAfterUpdate = "60"
$iIniFail += 1
$iIniError = $iIniError & "TelnetDelayAfterUpdate, "
ElseIf $aTelnetDelayAfterUpdate < 1 Then
$aTelnetDelayAfterUpdate = 1
FileWriteLine($aLogFile, _NowCalc() & " [Notice] Telnet server-is-alive check interval was out of range. Interval set to: " & $aTelnetDelayAfterUpdate & " minutes.")
ElseIf $aTelnetDelayAfterUpdate > 360 Then
$aTelnetDelayAfterUpdate = 360
FileWriteLine($aLogFile, _NowCalc() & " [Notice] Telnet server-is-alive check interval was out of range. Interval set to: " & $aTelnetDelayAfterUpdate & " minutes.")
EndIf
...
FileWriteLine($aLogFile, _NowCalc() & " [Watchdog] Watchdog wait for server to start was out of range. Interval set to: " & $aWatchdogWaitServerStart & " minutes.")
...
FileWriteLine($sIniFile, @CRLF)
IniWrite($sIniFile, " --------------- KEEP ALIVE WATCHDOG ---------------", "Pause watchdog for _ minutes after server updated to allow map generation (1-360) ###", $aTelnetDelayAfterUpdate)
...
IniWrite($sIniFile, " --------------- DISCORD INTEGRATION --------------- ", "Online Player Message (\o - Online Player Count, \m - Max Players) ###", $sDiscordPlayersMsg)
