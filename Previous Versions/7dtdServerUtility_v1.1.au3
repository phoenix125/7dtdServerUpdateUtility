...
While True ;**** Loop Until Closed ****
	...
	If Not ProcessExists($g_s7dtdPID) Then
		...
		FileWriteLine($g_c_sLogFile, _NowCalc() & " 7DTD Dedicated Server Started [" & $g_c_sServerEXE & $LogTimeStamp & " -quit -batchmode -nographics -configfile=" & $ConfigFile & " -dedicated ]")
	...
	If ($sUseDiscordBot = "yes") Or ($sUseTwitchBot = "yes") Or ($g_sUseMCRCON = "yes") Then
		If $g_iBeginDelayedShutdown = 1 Then
			...
			Local $sShutdownMessage = $aServerName & " Restarting in " & $g_iDelayShutdownTime & " minutes"
			If $g_sUseMCRCON = "yes" Then
