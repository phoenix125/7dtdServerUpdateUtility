...
#AutoIt3Wrapper_Outfile=Builds\7dtdServerUtility_v1.4.1.exe
...
;   EndIf
;EndFunc
#EndRegion ;**** Backup Server Files Before Update **** (WORK IN PROGRESS)
...
Func ReadUini($sIniFile, $sLogFile)
	...
	If $iniCheck = $sAnnounceUpdateMessage Then
		$sAnnounceUpdateMessage = "Yay! New server update!"
		$iIniFail += 1
	EndIf
...
While True ;**** Loop Until Closed ****
	...
	If Not ProcessExists($g_s7dtdPID) Then
		...
		Else
			...
			SplashTextOn("Restarting Server", "Restarting server to apply config change(s)." & @CRLF & "Server name: " & $aServerNameVer & @CRLF & "Game Name: " & $aGameName & @CRLF & @CRLF & "Please wait 1-2 minutes.")
			Global $aRebootMe = 1
			CloseServer()
		EndIf
		; **** Append Server Version to Server Name And/Or Change GameName to Server Version ****
