...
#AutoIt3Wrapper_Outfile=Builds\7dtdServerUpdateUtility_v1.8.3.exe
#AutoIt3Wrapper_Outfile_x64=Builds\7dtdServerUpdateUtility_x64_v1.8.3.exe
...
$aUtilVersion = "v1.8.3" ; (2019-01-27)
...
Func SendInGame($ip, $port, $pass, $g_sInGameMessage)
	If $aUsePuttytel = "yes" Then
		FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] Send Message In Game: " & $g_sInGameMessage)
		Local $sFileExists = FileExists(@ScriptDir & "\puttytel.exe")
		If $sFileExists = 0 Then
			InetGet("https://the.earth.li/~sgtatham/putty/latest/w32/puttytel.exe", @ScriptDir & "\puttytel.exe", 0)
			If Not FileExists(@ScriptDir & "\puttytel.exe") Then
	...
	Else
		...
		Sleep(250)
		If $socket = -1 Then
			FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] Send Message In Game FAILED. No response from Telnet.")
		EndIf
		TCPSend($socket, @CRLF & $pass & @CRLF)
		For $i = 1 To 90
			$aRecv = TCPRecv($socket, 1024)
