...
#AutoIt3Wrapper_Outfile=Builds\7dtdServerUpdateUtility_v1.8.2.exe
#AutoIt3Wrapper_Outfile_x64=Builds\7dtdServerUpdateUtility_x64_v1.8.2.exe
...
$aUtilVersion = "v1.8.2" ; (2019-01-26)
...
Global Const $g_c_sUtilityVer = "7dtdServerUpdateUtility" & " " & $aUtilVersion
Global Const $g_c_sServerEXE = "7DaysToDieServer.exe"
Global Const $g_c_sPIDFile = @ScriptDir & "\7dtdServerUpdateUtility_lastpid.tmp"
Global Const $g_c_sLogFile = @ScriptDir & "\7dtdServerUpdateUtility.log"
	...
	If $aUsePuttytel = "yes" Then
		...
		If FileExists(@ScriptDir & "\puttytel.exe") Then
			...
			If $CrashCheck = 0 Then
				...
				ControlSend($g_IP & " - PuTTYtel", "", "", "exit")
				WinWait("PuTTYtel Fatal Error", "", 5)
				If ProcessExists("puttytel.exe") Then
					ProcessClose("puttytel.exe")
				EndIf
...
Func TelnetCheck($ip, $port, $pass)
	If $aUsePuttytel = "yes" Then
		...
		If $sFileExists = 0 Then
			InetGet("https://the.earth.li/~sgtatham/putty/latest/w32/puttytel.exe", @ScriptDir & "\puttytel.exe", 0)
			If Not FileExists(@ScriptDir & "\puttytel.exe") Then
				FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] Telnet KeepAlive check: Could not find puttytel.exe at " & @ScriptDir)
				Exit
			EndIf
		EndIf
		If FileExists(@ScriptDir & "\puttytel.exe") Then
...
Func iniFileCheck($sIniFile, $iIniFail)
	If FileExists($sIniFile) Then
		...
		UpdateIni($sIniFile)
		FileWriteLine($g_c_sLogFile, _NowCalc() & " INI MISMATCH: Found " & $iIniFail & " missing variable(s) in 7dtdServerUpdateUtility.ini. Backup created and all existing settings transfered to new INI. Please modify INI and restart.")
...
SplashTextOn("7dtdServerUtil", "7dtdServerUpdateUtility started.", 200, 50, -1, -1, $DLG_MOVEABLE, "")
...
If $aServerTelnetPass = "CHANGEME" Or $aServerTelnetPass = "" Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " . . . Server Telnet Password was " & $aServerTelnetPass & ". Password CHANGED to: 7dtdServerUpdateUtility")
		...
		Switch @error
			Case 0
				...
				If ProcessExists($g_s7dtdPID) And ($g_iBeginDelayedShutdown = 0) Then
					...
					If ($sUseDiscordBotScheduled = "yes") Or ($sUseDiscordBotUpdate = "yes") Or ($sUseTwitchBotScheduled = "yes") Or ($sUseTwitchBotUpdate = "yes") Or ($sInGameAnnounce = "yes") Then
						Local $aMaintenanceMsg = """WARNING! Server maintenance restart begins in " & $g_iDelayShutdownTime & " minutes...""" & @CRLF
	...
	If ($sUseDiscordBotScheduled = "yes") Or ($sUseDiscordBotUpdate = "yes") Or ($sUseTwitchBotScheduled = "yes") Or ($sUseTwitchBotUpdate = "yes") Or ($sInGameAnnounce = "yes") Then
		If $g_iBeginDelayedShutdown = 1 Then
			...
			If $aBotMsg = $aMaintenanceMsg Then
				$sShutdownMessage = $aServerName & ": Server maintenance. Restarting in " & $g_iDelayShutdownTime & " minutes"
				If $sInGameAnnounce = "yes" Then
					SendInGame($g_IP, $aServerTelnetPort, $aServerTelnetPass, $aMaintenanceMsg)
				EndIf
