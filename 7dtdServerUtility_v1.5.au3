...
#AutoIt3Wrapper_Outfile=Builds\7dtdServerUtility_v1.5.exe
#AutoIt3Wrapper_Outfile_x64=Builds\7dtdServerUtility_x64_v1.4.exe
...
Func GetLatestVersion($sCmdDir)
	...
	Else
		...
		If $ServerVer = 0 Then
			...
			If $g_bDebug Then
				FileWriteLine($g_c_sLogFile, _NowCalc() & " . . . Latest Stable Version: " & $hBuildID)
			EndIf
		Else
			FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $aServerName & " (PID: " & $g_s7dtdPID & ")] Update Check via 7DTD Experimental Branch")
			If $g_bDebug Then
				FileWriteLine($g_c_sLogFile, _NowCalc() & " . . . Latest Experimental Version: " & $hBuildID)
			EndIf
		EndIf
		FileClose($hFileOpen)
...
Func GetInstalledVersion($sGameDir)
	...
	EndIf
	If $g_bDebug Then
		FileWriteLine($g_c_sLogFile, _NowCalc() & " . . . . . Installed Version: " & $aAppInfo[2])
	EndIf
	Return $aReturn
...
SplashTextOn("7dtdServerUtil", "7dtdServerUtility started.", 450, 125, -1, -1, $DLG_MOVEABLE, "")
...
While True ;**** Loop Until Closed ****
	...
	If Not ProcessExists($g_s7dtdPID) Then
		...
		Else
			...
			SplashTextOn("7dtdServerUtil", "Restarting server to apply config change(s)." & @CRLF & "Server name: " & $aServerNameVer & @CRLF & "Game Name: " & $aGameName & @CRLF & @CRLF & "Please wait 1-2 minutes.", 450, 125, -1, -1, $DLG_MOVEABLE, "")
			Global $aRebootMe = 1
			CloseServer()
		EndIf
		; **** Append Server Version to Server Name And/Or Change GameName to Server Version ****
