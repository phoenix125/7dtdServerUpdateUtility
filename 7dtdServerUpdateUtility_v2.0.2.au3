...
#AutoIt3Wrapper_Outfile=Builds\7dtdServerUpdateUtility_v2.0.2.exe
#AutoIt3Wrapper_Outfile_x64=Builds\7dtdServerUpdateUtility_x64_v2.0.2.exe
...
#AutoIt3Wrapper_Res_ProductVersion=2.0.2
#AutoIt3Wrapper_Res_CompanyName=http://www.Phoenix125.com
#AutoIt3Wrapper_Res_LegalCopyright=http://www.Phoenix125.com
#AutoIt3Wrapper_Res_Language=1033
#EndRegion ;**** Directives created by AutoIt3Wrapper_GUI ****

$aUtilVersion = "v2.0.2" ; (2019-02-06)
...
Func ReadUini($sIniFile, $sLogFile)
	...
	Global $sAnnounceUpdateMessage = IniRead($sIniFile, " --------------- ANNOUNCEMENT CONFIGURATION --------------- ", "Announcement Updates (\m - minutes)", $iniCheck)
	Global $sAnnounceRemoteRestartMessage = IniRead($sIniFile, " --------------- ANNOUNCEMENT CONFIGURATION --------------- ", "Announcement Remote Restart (\m - minutes)", $iniCheck)
	Global $sInGameAnnounce = IniRead($sIniFile, " --------------- ANNOUNCEMENT CONFIGURATION --------------- ", "Announce messages in-game? (Requires telnet) (yes/no)", $iniCheck)
	Global $sUseDiscordBotDaily = IniRead($sIniFile, " --------------- DISCORD INTEGRATION --------------- ", "Send Discord message for scheduled restarts? (yes/no)", $iniCheck)
	Global $sUseDiscordBotUpdate = IniRead($sIniFile, " --------------- DISCORD INTEGRATION --------------- ", "Send Discord message for update restarts? (yes/no)", $iniCheck)
	Global $sUseDiscordBotRemoteRestart = IniRead($sIniFile, " --------------- DISCORD INTEGRATION --------------- ", "Send Discord message for Remote Restart restarts? (yes/no)", $iniCheck)
	...
	Global $sUseTwitchBotDaily = IniRead($sIniFile, " --------------- TWITCH INTEGRATION --------------- ", "Send Twitch message for scheduled restarts? (yes/no)", $iniCheck)
	Global $sUseTwitchBotUpdate = IniRead($sIniFile, " --------------- TWITCH INTEGRATION --------------- ", "Send Twitch message for update restarts? (yes/no)", $iniCheck)
	Global $sUseTwitchBotRemoteRestart = IniRead($sIniFile, " --------------- TWITCH INTEGRATION --------------- ", "Send Twitch message for Remote Restart restarts? (yes/no)", $iniCheck)
	...
	Global $aValidate = IniRead($sIniFile, " --------------- " & StringUpper($aUtilityName) & " MISC OPTIONS --------------- ", "Validate files with SteamCMD update? (yes/no)", $iniCheck)
	Global $aUpdateSource = IniRead($sIniFile, " --------------- " & StringUpper($aUtilityName) & " MISC OPTIONS --------------- ", "For update checks, use (0)SteamCMD or (1)SteamDB.com", $iniCheck)
	Global $aUsePuttytel = IniRead($sIniFile, " --------------- " & StringUpper($aUtilityName) & " MISC OPTIONS --------------- ", "Use puttytel for telnet client? (yes/no)", $iniCheck)
	Global $sObfuscatePass = IniRead($sIniFile, " --------------- " & StringUpper($aUtilityName) & " MISC OPTIONS --------------- ", "Hide passwords in log files? (yes/no)", $iniCheck)
	Global $aExternalScriptHideYN = IniRead($sIniFile, " --------------- " & StringUpper($aUtilityName) & " MISC OPTIONS --------------- ", "Hide external scripts when executed? (if yes, scripts may not execute properly) (yes/no)", $iniCheck)
	Global $aDebug = IniRead($sIniFile, " --------------- " & StringUpper($aUtilityName) & " MISC OPTIONS --------------- ", "Enable debug to output more log detail? (yes/no)", $iniCheck)
	...
	If $iniCheck = $sAnnounceNotifyTime3 Then
		...
		$iIniFail += 1
	EndIf
	If $iniCheck = $sAnnounceDailyMessage Then
		$sAnnounceDailyMessage = "WARNING! Server is rebooting in \m minutes."
		$iIniFail += 1
	EndIf
	If $iniCheck = $sAnnounceUpdateMessage Then
		$sAnnounceUpdateMessage = "Fun Pimps have released a new update. Server is rebooting in \m minutes."
		$iIniFail += 1
	EndIf
	If $iniCheck = $sAnnounceRemoteRestartMessage Then
		$sAnnounceRemoteRestartMessage = "Admin has requested a server reboot. Server is rebooting in \m minutes."
		$iIniFail += 1
	EndIf
	...
	If $sUseDiscordBotRemoteRestart = "yes" Or $sUseDiscordBotDaily = "yes" Or $sUseDiscordBotUpdate = "yes" Or $sUseTwitchBotRemoteRestart = "yes" Or $sUseTwitchBotDaily = "yes" Or $sUseTwitchBotUpdate = "yes" Or $sInGameAnnounce = "yes" Then
		Global $aDailyMsg = AnnounceReplaceTime($sAnnounceNotifyTime1, $sAnnounceDailyMessage)
		Global $aDailyTime = StringSplit($sAnnounceNotifyTime1, ",")
		Global $aDailyCnt = Int($aDailyTime[0])
		Global $aUpdateMsg = AnnounceReplaceTime($sAnnounceNotifyTime2, $sAnnounceUpdateMessage)
		Global $aUpdateTime = StringSplit($sAnnounceNotifyTime2, ",")
		Global $aUpdateCnt = Int($aUpdateTime[0])
		Global $aRemoteMsg = AnnounceReplaceTime($sAnnounceNotifyTime3, $sAnnounceRemoteRestartMessage)
...
Func UpdateIni($sIniFile)
	FileWriteLine($sIniFile, "[ --------------- " & StringUpper($aUtilityName) & " INFORMATION --------------- ]")
	...
	FileWriteLine($sIniFile, "(sample: http://192.168.1.10:57520/?restart=password)")
	FileWriteLine($sIniFile, @CRLF)
	...
	FileWriteLine($sIniFile, @CRLF)
	IniWrite($sIniFile, " --------------- ANNOUNCEMENT CONFIGURATION --------------- ", "Announce messages in-game? (Requires telnet) (yes/no)", $sInGameAnnounce)
	...
	IniWrite($sIniFile, " --------------- ANNOUNCEMENT CONFIGURATION --------------- ", "Announcement Daily (\m - minutes)", $sAnnounceDailyMessage)
	IniWrite($sIniFile, " --------------- ANNOUNCEMENT CONFIGURATION --------------- ", "Announcement Updates (\m - minutes)", $sAnnounceUpdateMessage)
	IniWrite($sIniFile, " --------------- ANNOUNCEMENT CONFIGURATION --------------- ", "Announcement Remote Restart (\m - minutes)", $sAnnounceRemoteRestartMessage)
	FileWriteLine($sIniFile, @CRLF)
	IniWrite($sIniFile, " --------------- DISCORD INTEGRATION --------------- ", "Send Discord message for scheduled restarts? (yes/no)", $sUseDiscordBotDaily)
	IniWrite($sIniFile, " --------------- DISCORD INTEGRATION --------------- ", "Send Discord message for update restarts? (yes/no)", $sUseDiscordBotUpdate)
	IniWrite($sIniFile, " --------------- DISCORD INTEGRATION --------------- ", "Send Discord message for Remote Restart restarts? (yes/no)", $sUseDiscordBotRemoteRestart)
	;	IniWrite($sIniFile, " --------------- DISCORD INTEGRATION --------------- ", "Append server name to beginning of messages? (yes/no)", $sUseDiscordBotAppendServer)
	...
	FileWriteLine($sIniFile, @CRLF)
	IniWrite($sIniFile, " --------------- TWITCH INTEGRATION --------------- ", "Send Twitch message for scheduled restarts? (yes/no)", $sUseTwitchBotDaily)
	IniWrite($sIniFile, " --------------- TWITCH INTEGRATION --------------- ", "Send Twitch message for update restarts? (yes/no)", $sUseTwitchBotUpdate)
	IniWrite($sIniFile, " --------------- TWITCH INTEGRATION --------------- ", "Send Twitch message for Remote Restart restarts? (yes/no)", $sUseTwitchBotRemoteRestart)
	...
	FileWriteLine($sIniFile, @CRLF)
	IniWrite($sIniFile, " --------------- " & StringUpper($aUtilityName) & " MISC OPTIONS --------------- ", "Validate files with SteamCMD update? (yes/no)", $aValidate)
	IniWrite($sIniFile, " --------------- " & StringUpper($aUtilityName) & " MISC OPTIONS --------------- ", "For update checks, use (0)SteamCMD or (1)SteamDB.com", $aUpdateSource)
	IniWrite($sIniFile, " --------------- " & StringUpper($aUtilityName) & " MISC OPTIONS --------------- ", "Use puttytel for telnet client? (yes/no)", $aUsePuttytel)
	IniWrite($sIniFile, " --------------- " & StringUpper($aUtilityName) & " MISC OPTIONS --------------- ", "Hide passwords in log files? (yes/no)", $sObfuscatePass)
	IniWrite($sIniFile, " --------------- " & StringUpper($aUtilityName) & " MISC OPTIONS --------------- ", "Hide external scripts when executed? (if yes, scripts may not execute properly) (yes/no)", $aExternalScriptHideYN)
	IniWrite($sIniFile, " --------------- " & StringUpper($aUtilityName) & " MISC OPTIONS --------------- ", "Enable debug to output more log detail? (yes/no)", $aDebug)
EndFunc   ;==>UpdateIni
	...
	If (($aRestartDaily = "yes") And ((_DateDiff('n', $aTimeCheck2, _NowCalc())) >= 1) And (DailyRestartCheck($aRestartDays, $aRestartHours, $aRestartMin)) And ($aBeginDelayedShutdown = 0)) Then
		If ProcessExists($aServerPID) Then
			...
			If $aDelayShutdownTime Not = 0 Then
				;			If ($sUseDiscordBotDaily = "yes") Or ($sUseDiscordBotUpdate = "yes") Or ($sUseTwitchBotDaily = "yes") Or ($sUseTwitchBotUpdate = "yes") Or ($sInGameAnnounce = "yes") Then
				;				Global $aBotMsg = $sAnnounceDailyMessage
				$aBeginDelayedShutdown = 1
				$aRebootReason = "daily"
				$aTimeCheck0 = _NowCalc
	...
	If $aDelayShutdownTime Not = 0 Then
		If $aBeginDelayedShutdown = 1 Then
			...
			If $aRebootReason = "daily" Then
				...
				$sShutdownMessage = $aDailyMsg[$aAnnounceCount0]
				...
				If $sInGameAnnounce = "yes" Then
					SendInGame($aServerIP, $aTelnetPort, $aTelnetPass, $sShutdownMessage)
				EndIf
			...
			If $aRebootReason = "remoterestart" Then
				...
				$sShutdownMessage = $aRemoteMsg[$aAnnounceCount0]
			...
			If $aRebootReason = "update" Then
				...
				$sShutdownMessage = $aUpdateMsg[$aAnnounceCount0]
				$aDelayShutdownTime = $aUpdateTime[$aAnnounceCount0] - $aUpdateTime[$aAnnounceCount1]
				$aAnnounceCount1 = $aAnnounceCount0 - 1
				If $aAnnounceCount1 = 0 Then
					;					$aDelayShutdownTime = $aUpdateTime[$aAnnounceCount0]
					$aDelayShutdownTime = 0
					$aBeginDelayedShutdown = 3
				Else
					$aDelayShutdownTime = $aUpdateTime[$aAnnounceCount0] - $aUpdateTime[$aAnnounceCount1]
		...
		ElseIf ($aBeginDelayedShutdown = 2) And (_DateDiff('n', $aTimeCheck0, _NowCalc()) >= $aDelayShutdownTime) Then ; **** REPEAT ANNOUNCEMENTS UNTIL LAST COUNT ***
			...
			If $aRebootReason = "daily" Then
				$sShutdownMessage = $aDailyMsg[$aAnnounceCount1]
			...
			EndIf
			If $aRebootReason = "remoterestart" Then
				$sShutdownMessage = $aRemoteMsg[$aAnnounceCount1]
				If $aAnnounceCount1 > 1 Then
			...
			EndIf
			If $aRebootReason = "update" Then
				$sShutdownMessage = $aUpdateMsg[$aAnnounceCount1]
				If $aAnnounceCount1 > 1 Then
